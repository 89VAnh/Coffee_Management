...
namespace Anh_Coffee.View
{
    public partial class frmCategoryFood : Form
    {
        ...
        {
            UpdateDgv(categoryFoodList.Where(f => f.Name.ToLower().Contains(txtSearch.Text)).ToList());
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (selectedCategoryFoodID > 0)
            {
                CategoryFood cF = categoryFoodList.SingleOrDefault(x => x.ID == selectedCategoryFoodID);
